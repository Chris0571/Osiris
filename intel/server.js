...
// ════════════════════════════════════════════════════
// §7.1 — RECON SCANNER ENDPOINT (/scan)
// ════════════════════════════════════════════════════
...
app.get('/scan', async (req, res) => {
  ...
  if (process.env.OSIRIS_KEY && apiKey !== process.env.OSIRIS_KEY) {
    return res.status(401).json({ error: 'Unauthorized scanner key' });
  }
  if (!target) {
    return res.status(400).json({ error: 'Missing target parameter' });
  }

  // Scansione TCP rapida sulle porte più comuni
  const commonPorts = [21, 22, 53, 80, 443, 3000, 8080];
  const openPorts = [];
  ...
  res.json({
    ...
    message: `Scanned common ports on ${target}`,
    timestamp: new Date().toISOString()
  });
});
